...
- **Blockers:** none
- **Just landed:** Core: Users + Auth — JWT login, roles (owner/manager/worker), route protection, role-driven Dashboard, CLI seed script; a `run.bat` one-click local dev runner (seeds a `demo`/`demo1234` account); role-scope decision — manager-only in practice for now, worker deferred — see Decisions Log
